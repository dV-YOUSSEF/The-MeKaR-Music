...
from iLBaReD import YouTube, app
from iLBaReD.core.call import Omar
from iLBaReD.misc import SUDOERS, db
...
from iLBaReD.utils.decorators.language import languageCB
from iLBaReD.utils.formatters import seconds_to_min
from iLBaReD.utils.inline import close_markup, stream_markup, stream_markup_timer
from iLBaReD.utils.stream.autoclear import auto_clean
from iLBaReD.utils.thumbnails import get_thumb
from config import (
    BANNED_USERS,
    SUPPORT_CHAT,
    SOUNCLOUD_IMG_URL,
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Omar.pause_stream(chat_id)
        ...
        if command == "Skip":
            txt = f"قام : ⦗ {mention} ⦘ بتخطي البث 😋♥️ ،"
            popped = None
            try:
                popped = check.pop(0)
                if popped:
                ...
                if not check:
                    ...
                    try:
                        return await Omar.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
            ...
            txt = f"قام : ⦗ {mention} ⦘ بئعادة تشغيل البث 😋♥️ ،"
        await CallbackQuery.answer()
        queued = check[0]["file"]
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Omar.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await Omar.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await Omar.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Omar.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
